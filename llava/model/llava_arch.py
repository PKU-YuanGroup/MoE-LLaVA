...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_image_tower") or hasattr(config, "mm_video_tower"):
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        # image_features = []
        # # if type(images) is list or images.ndim == 5:  # FIXME: why images.ndim == 5 ?
        # if type(images) is list:  # mixup videos and images
        #     '''
        ...
        #     '''
        #     for i in images:
        #         if i.ndim == 3:  # a image, [c, h, w]
        #             image_features.append(self.encode_images(i.unsqueeze(0)).to(self.device)[0])  # [n, c]
        #         elif i.ndim == 4:  # a video, [t, c, h, w]
        #             video_features = self.encode_videos(i.unsqueeze(0)).to(self.device)[0]  # [t, 1+1+n_, c]
        #             for v_frame in video_features:
        #                 image_features.append(v_frame)  # fake images
        #         else:
        #             raise NotImplementedError(f'We do not know the shape of {images.shape}')
        #
        # elif images.ndim == 4:  # batch consists of images, [b, c, h, w]
        #     image_features = self.encode_images(images).to(self.device)
        #
        # elif images.ndim == 5:  # batch consists of videos, [b, t, c, h, w]
        #     video_features = self.encode_videos(images).to(self.device)  # [b, t, 1+1+n_, c]
        #     video_features = rearrange(video_features, 'b t new_n c -> (b t) new_n c')  # fake images
        #     for v_frame in video_features:
        #         image_features.append(v_frame)  # fake images
        #
        # else:
        ...
        if type(images) is list:  # mixup videos and images
            ...
            for i in images:
                if i.ndim == 3:  # a image, [c, h, w]
                    if image_tower is not None:
                        ...
                        image_features.append(i.unsqueeze(0).to(self.device)[0])  # dummy feature
                elif i.ndim == 4:  # a video, [t, c, h, w]
                    video_features = self.encode_videos(i.unsqueeze(0)).to(self.device)[0]  # [t, 1+1+n_, c]
                    num_frames = video_features.shape[0]
                    for t_i in range(num_frames):
                        image_features.append(video_features[t_i, :, :])  # fake images
                else:
                    raise NotImplementedError(f'We do not know the shape of {images.shape}')
            ...
            if image_tower is not None:
                ...
                image_features = images  # dummy feature

        elif images.ndim == 5:  # batch consists of videos, [b, t, c, h, w]
            # print('video batch', images.shape)
            video_features = self.encode_videos(images).to(self.device)  # [b, t, 1+1+n_, c]
            # video_features = rearrange(video_features, 'b t new_n c -> (b t) new_n c')  # fake images
            # for v_frame in video_features:
            #     image_features.append(v_frame)  # fake images
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if num_images == 0:
                ...
                # cur_input_embeds = torch.cat([cur_input_embeds_1, torch.zeros_like(cur_input_embeds_1, dtype=cur_image_features.dtype, device=cur_image_features.device)[0:0]], dim=0)
