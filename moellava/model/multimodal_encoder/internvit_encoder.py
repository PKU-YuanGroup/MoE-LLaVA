...
from .intern_vit_model import InternVisionModel 
from .intern_vit_model import InternVisionConfig
...
class InternViTVisionTower(nn.Module):
    def __init__(self, image_tower, args, delay_load=False, cache_dir='./cache_dir'):
        ...
        if not delay_load:
            ...
            self.cfg_only = InternVisionConfig.from_pretrained(self.image_tower_name, cache_dir=self.cache_dir)

    def load_model(self):
        self.image_processor = CLIPImageProcessor.from_pretrained(self.image_tower_name, cache_dir=self.cache_dir)
        self.image_tower = InternVisionModel.from_pretrained(self.image_tower_name, cache_dir=self.cache_dir)
